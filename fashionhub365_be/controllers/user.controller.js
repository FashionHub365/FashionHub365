...
const { fileUploadService, userService } = require('../services');
...
const updateProfile = catchAsync(async (req, res) => {
    ...
    const profile = {};
    ['full_name', 'phone', 'gender', 'bio', 'avatar_url'].forEach((field) => {
    ...
    if (req.body.dob) {
        profile.dob = new Date(req.body.dob);
    }

    if (req.file) {
        const uploadResult = await fileUploadService.uploadImage(req.file, 'users/avatars');
    ...
    if (Object.keys(updateBody).length === 0) {
        throw new ApiError(httpStatus.BAD_REQUEST, 'No profile changes provided');
    }
