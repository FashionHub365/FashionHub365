...
export const Login = () => {
    ...
    const { login, verifyOtpLogin, googleLogin } = useAuth();
    ...
    const handleCredentialSubmit = async (e) => {
        ...
        if (result.success) {
            if (result.requiresOtp) {
                setPendingEmail(result.email || identifier);
                setIsOtpStep(true);
                ...
                <div className="w-full md:w-1/2 p-8 md:p-12 bg-white dark:bg-gray-900 relative flex flex-col justify-center text-left">
                    <div className="text-left mb-10">
                        ...
                        <p className="text-gray-500 dark:text-gray-400">
                            {isOtpStep ? 'Enter the 6-digit code sent to your email.' : 'Please enter your details to sign in.'}
                        </p>
                    </div>
                    ...
                    {!isOtpStep ? (
                        ...
                        <form className="space-y-6" onSubmit={handleOtpSubmit}>
                            ...
                            <button
                                type="submit"
                                disabled={isLoading}
                                className={`w-full flex justify-center py-3 px-4 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none transition-all duration-300 ${isLoading ? 'opacity-70 cursor-not-allowed' : ''}`}
                            >
