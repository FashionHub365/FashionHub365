...
export const StoreProfile = () => {
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      // Fixed storeId filter
      const response = await listingApi.getProducts({ ...cleanParams, storeId });
      if (response.success) {
            ...
            {moreCategories.length > 0 && (
              <div className="relative" ref={moreMenuRef}>
                <button 
                  ...
                  className={`px-8 py-4 text-sm font-medium text-gray-800 hover:text-[#ee4d2d] transition-colors whitespace-nowrap uppercase tracking-tight flex items-center gap-1 border-b-2 ${
                    moreCategories.some(cat => cat.name === filters.category)
                    ? "border-[#ee4d2d] text-[#ee4d2d]"
                    : "border-transparent"
                  }`}
                >
